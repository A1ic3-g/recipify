    ...
    for i in range(0,len(query)):
        elements += '''<div class="container justify-content-center d-flex" style="padding: 3%">
            <div class="card border-primary mb-3" style="width: 75%;">
                <a href="/recipe?label={label}&cuisineType={cuisineType}&mealType={mealType}&url={url}"><div class="card-body">
                    <div class="row">
                        <div class="col-sm">
        ...
        </div> '''.format(cuisineType=",".join(query[i]['cuisineType']), mealType=",".join(query[i]['mealType']), url=query[i]['url'], image=query[i]['image'], label=query[i]['label'])  # Add the correct details in the correct location
...
def recipe_page():
    ...
    cuisines = request.args.get("cuisineType").split(',')
    meals = request.args.get("mealType").split(',')
    recipe = {
        "label": label,
        "url": url,
